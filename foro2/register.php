...
//CODIGO CUSTOM COMIENZO PAPU

include 'ChromePhp.php';

//CODIGO CUSTOM FINAL PAPU
...
if (isset($_POST['form_sent']))
{
	...
	$form_invite_code = pun_trim($_POST['req_code']);
	...
	ChromePhp::log("el de result incluido");
	...
	//$invitation_code_sql='code=\''.$db->escape($form_invite_code).'\'';
	$code_valid_result = $db->query('SELECT code FROM '.$db->prefix.'invitation_codes WHERE code=\''.$db->escape($form_invite_code).'\'') or error('Unable to fetch code info', __FILE__, __LINE__, $db->error());
	//ChromePhp::log($invitation_code_sql);
	ChromePhp::log("Hola");
	ChromePhp::log($code_valid_result);

	if (!$db->num_rows($code_valid_result)) {		
		$errors[] = $lang_register['Invalid Invitation Code'];
		message($lang_register['Invalid Invitation Code']);
	}
	else{
		$db->query('DELETE FROM '.$db->prefix.'invitation_codes WHERE code=\''.$db->escape($form_invite_code).'\'');
	}
...
$required_fields = array('req_user' => $lang_common['Username'], 'req_code' => $lang_common['InviteCode'],'req_password1' => $lang_common['Password'], 'req_password2' => $lang_prof_reg['Confirm pass'], 'req_email1' => $lang_common['Email'], 'req_email2' => $lang_common['Email'].' 2');
	...
	<div class="box">
		<form id="register" method="post" action="register.php?action=register" onsubmit="this.register.disabled=true;if(process_form(this)){return true;}else{this.register.disabled=false;return false;}">
			<div class="inform">
                ...
                <fieldset>
					<legend><?php echo $lang_register['InviteCode legend'] ?></legend>
					<div class="infldset">
						<input type="hidden" name="form_sent" value="1" />
						<label class="required"><strong><?php echo $lang_common['InviteCode'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="req_code" value="<?php if (isset($_POST['req_code'])) echo pun_htmlspecialchars($_POST['req_code']); ?>" size="10" maxlength="10" /><br /></label>
					</div>
				</fieldset>
			</div>
