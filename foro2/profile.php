...
<div class="blockform">
	...
	<div class="box">
		<form id="change_pass" method="post" action="profile.php?action=change_pass&amp;idf=<?php echo $id ?>" onsubmit="return process_form(this)">
			<div class="inform">
...
//COMIENZO CODIGO CUSTOM PAPU----------------------------------------

else if ($action == 'gen_code')
{
$page_title = array(pun_htmlspecialchars($pun_config['o_board_title']), $lang_common['Profile'], $lang_profile['Generate Code']);
	define('PUN_ACTIVE_PAGE', 'profile');
	require PUN_ROOT.'header.php';
    ...
    $db->query('INSERT INTO '.$db->prefix.'invitation_codes (code,tiempo_creacion) VALUES(\''.$randomString.'\',NOW())');


    

    $gen_code_field = '<label class="required"><strong>'.	$lang_common['InviteCode'].' <span>'.$lang_common['Required'].'</span></strong><br /><input type="text" name="req_code" id="gen_code_field" value="'.$randomString.'" size="10" maxlength="10" readonly/><br /></label>'."\n";

    /*$gen_code_inputs="<script> function generate_code(){	 var temp = document.getElementById('gen_code_field');
							 temp.value = '".$randomString."';
							 return true;
							}
                            </script>";*/
...
<div class="blockform">
	<h2><span><?php echo $lang_profile['Generate Code'] ?></span></h2>
	<div class="box">
		<?php //echo $gen_code_inputs ?>
		<form id="generate_code" method="post" enctype="multipart/form-data" action="profile.php?action=gen_code&amp;id=<?php echo $id ?>" onsubmit="return  generate_code()">
			<div class="inform">
				<fieldset>
						<legend><?php echo $lang_prof_reg['GenInviteCode legend'] ?></legend>
						<div class="infldset">
						<input type="hidden" name="form_sent" value="1" />
							<?php echo $gen_code_field ?>
...
else
{
  ...
  $gen_code_field = '<label class="required"><strong>'.	$lang_common['InviteCode'].' <span>'.$lang_common['Required'].'</span></strong><br /><input type="text" name="req_code" id="gen_code_field" value="'.pun_htmlspecialchars($user['InviteCode']).'" size="10" maxlength="10" /><br /></label>'."\n";
...
<?php endif; ?>						</div>
                ...
                <div class="inform">                
					<fieldset>
						<legend><?php echo $lang_profile['Generate Code'] ?></legend>
						<div class="infldset">
							<?php //echo $gen_code_field ?>
							<p><?php echo $lang_profile['GenInviteCode legend'] ?></p>
							<p class="actions"><span><a href="profile.php?action=gen_code&amp;id=<?php echo $id ?>"><?php echo $lang_profile['Generate Code'] ?></a></span></p>                              
						</div>
					</fieldset>					
				</div>
				<div class="inform">
